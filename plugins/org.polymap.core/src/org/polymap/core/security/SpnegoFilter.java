...
public class SpnegoFilter
    ...
    public void doFilter( ServletRequest request, ServletResponse response, FilterChain chain ) throws IOException,
         ...
         * 
         * If the server receives a request for an access-protected object, and if an
         * acceptable Authorization header has not been sent, the server responds
         * with a "401 Unauthorized" status code, and a "WWW-Authenticate:" header as
         * per the framework described in [RFC 2616]. The initial WWW-Authenticate
         * header will not carry any gssapi-data.
         */
        ...
        /**
         * A client may initiate a connection to the server with an "Authorization"
         * header containing the initial token for the server. This form will bypass
         * the initial 401 error from the server when the client knows that the
         * server will accept the Negotiate HTTP authentication type.
         */
        log.info( "Authorization header found, continuing negotiation..." );

        FileOutputStream out = null;
        try {
            File f = new File( Polymap.getWorkspacePath().toFile(), "auth-token.txt" );
            out = new FileOutputStream( f );
            out.write( header.substring( 10 ).getBytes() );
        }
        finally {
...
//        complex( httpResponse, header );     
    ...
    private void complex( HttpServletResponse httpResponse, String header ) throws ServletException {
        ...
        try {
            gssapiData = com.sun.org.apache.xml.internal.security.utils.Base64.decode( header
